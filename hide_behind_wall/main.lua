...
local function JudgeSquareNineArea(squarePts, targetPt)
    ...
    local areaName = nameMap[areaId]

    return areaId, areaName
end

local function RenderShadow(squarePts, targetPt)
    local areaId = JudgeSquareNineArea(squarePts, targetPt)
    local p1, p2=nil,nil
    if areaId == 2 then
        -- 正上方：左上、右上
        p1 = squarePts[1]
...
local function RenderShadow2(squarePts, targetPt)
    local areaId = JudgeSquareNineArea(squarePts, targetPt)  --TODO
...
function mod:updateShadowRender()
    ...
    for k, grid in pairs(grids) do
        ...
        RenderShadow(four_vertices,player.Position) 
        RenderShadow2(four_vertices,player.Position)    
    end
